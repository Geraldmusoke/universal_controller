...
public class ControllerWithMongo{
    ...
    public ResponseEntity<HotpotResponse> handleHotpotRequest(@RequestBody HotpotRequest request) {
        ...
        try {
            ...
            exists = mongoTemplate.exists(query, "codes");
            ...
            System.err.println("Caught a general MongoException: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new HotpotResponse("Caught a general MongoException: " + e.getMessage()));
        }
